...
// loaders chained before ns-vue-loader, but exclude some loaders that simply
...
module.exports = function (content) {
  ...
  parts.template = selectTemplate(parts, options)
  ...
  function getCSSExtractLoader (lang) {
    ...
    if (typeof op.extract === 'function') {
      ...
      if (!extractor) {
        throw new Error(
          '[ns-vue-loader] extractCSS: true requires extract-text-webpack-plugin ' +
  ...
  function selectTemplate (parts, options) {
    let template = parts.template
    if (options.build === 'native') {
      template = _.find(parts.templates, template => {
        return _.has(template.attrs, 'native')
      })
    } else if (options.build === 'web') {
      template = _.find(parts.templates, template => {
        return _.has(template.attrs, 'web')
      })
    }
